...
interface FormState {
  ...
  tender_yearBuilt: string; tender_purchaseDate: string; tender_purchasePrice: string;
  ...
  si_effects_currency: string; si_effects_amount: string;
  si_nav_currency: string; si_nav_amount: string;
  si_tpl_currency: string; si_tpl_amount: string;
  si_uninsured_currency: string; si_uninsured_amount: string;
  ...
  cover_commercialOtherDetails: string; cover_commercialPassenger: string;
  ...
  end_towing: string; end_racing: string; end_racingNames: string; end_mastValue: string;
  ...
  decl_mortgage: string; decl_mortgageDetails: string;
  decl_soleOwner: string; decl_otherOwners: string;
...
const BLANK: FormState = {
  ownerName: '', dob: '', address: '', city: '', stateCounty: '', zipPostcode: '', country: 'New Zealand',
  email: '', phone: '', occupation: '', boatingQualifications: '', boatingExperience: '', previousVessels: '',
  hasAdditional: false,
  addl_name: '', addl_dob: '', addl_address: '', addl_city: '', addl_stateCounty: '', addl_zipPostcode: '',
  addl_experience: '', addl_qualifications: '',
  vesselName: '', hullNumber: '', countryFlag: 'New Zealand', stateFlagUSA: '', registrationStateUSA: '',
  yearBuilt: '', length: '', manufacturer: '', model: '', vesselType: '', isConversion: 'No',
  constructionMaterials: [], constructionOther: '',
  datePurchased: '', pricePaid: '', lastSurveyDate: '', surveyorName: '',
  engine_manufacturer: '', engine_model: '', engine_yearBuilt: '', engine_hp: '',
  engine_fuel: 'Diesel', engine_serial: '', engine_count: '1', engine_maxSpeed: '',
  hasTender: false,
  tender_serial: '', tender_displaysName: 'Yes',
  tender_manufacturer: '', tender_model: '', tender_length: '',
  tender_yearBuilt: '', tender_purchaseDate: '', tender_purchasePrice: '',
  tenderMotor_manufacturer: '', tenderMotor_model: '', tenderMotor_yearBuilt: '',
  tenderMotor_hp: '', tenderMotor_fuel: 'Petrol', tenderMotor_serial: '', tenderMotor_locked: 'Yes',
  hasTrailer: false,
  trailer_makeModel: '', trailer_age: '', trailer_value: '',
  ...
  si_effects_currency: 'NZD', si_effects_amount: '',
  si_nav_currency: 'NZD', si_nav_amount: '',
  si_tpl_currency: 'NZD', si_tpl_amount: '',
  si_uninsured_currency: 'NZD', si_uninsured_amount: '',
  si_medical_currency: 'NZD', si_medical_amount: '',
  si_crew_currency: 'NZD', si_crew_amount: '',
  si_passenger_currency: 'NZD', si_passenger_amount: '',
  cover_fullyComp: 'Yes', cover_tplOnly: 'No', cover_privatePleasure: 'Yes',
  cover_commercial: 'No', cover_barebat: 'No', cover_captainCharter: 'No',
  cover_fishingCharter: 'No', cover_commercialOther: 'No', cover_commercialOtherDetails: '',
  cover_commercialPassenger: 'No', cover_maxPassengers: '',
  cover_inWater: 'No', cover_employedCrew: 'No', cover_maxCrew: '',
  cover_waterSkiing: 'No', cover_medicalExp: 'No', cover_salvage: 'No',
  end_agreedValue: 'No', end_breachWarranty: 'No', end_loanAmount: '',
  end_buildersRisk: 'No', end_commercialFishing: 'No', end_diving: 'No',
  ...
  decl_mortgage: 'No', decl_mortgageDetails: '',
  decl_soleOwner: 'Yes', decl_otherOwners: '',
  decl_disabilities: 'No', decl_disabilityDetails: '',
  preferredStartDate: '', previousInsurer: '', noClaimsBonus: '',
...
export default function MarineProposalForm() {
  ...
  const toggleArr = useCallback((field: 'constructionMaterials' | 'safetyEquipment' | 'autoExtLocations', val: string) => {
    setForm(prev => {
  ...
  const nextStep = () => { setStep(s => Math.min(s + 1, 7)); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  const prevStep = () => { setStep(s => Math.max(s - 1, 1)); window.scrollTo({ top: 0, behavior: 'smooth' }); };

  const handleSubmit = async () => {
    if (!form.declarationAccepted) { setSubmitError('Please accept the declaration to proceed.'); return; }
    setSubmitting(true);
    setSubmitError('');
    try {
  ...
  const step1 = (
    <div className="space-y-4">
      <Card title="Personal Information">
        ...
        <Grid2>
          <Field label="City / Town" id="city" value={form.city} onChange={v => set('city', v)} required />
          <Field label="State / County (if applicable)" id="stateCounty" value={form.stateCounty} onChange={v => set('stateCounty', v)} />
        </Grid2>
        <Grid2>
          <Field label="Postcode / Zip" id="zipPostcode" value={form.zipPostcode} onChange={v => set('zipPostcode', v)} />
      ...
      <Card title="Contact Details">
        <Grid2>
          ...
          <Field label="Phone Number" id="phone" type="tel" value={form.phone} onChange={v => set('phone', v)} required placeholder="+64 21 000 0000" />
        </Grid2>
      ...
      {form.hasAdditional ? (
        <>
          <Card title="Additional Insured — Personal Details">
            ...
            <Grid2>
              <Field label="City / Town" id="addl_city" value={form.addl_city} onChange={v => set('addl_city', v)} />
              <Field label="State / County" id="addl_stateCounty" value={form.addl_stateCounty} onChange={v => set('addl_stateCounty', v)} />
            </Grid2>
            <Field label="Postcode / Zip" id="addl_zipPostcode" value={form.addl_zipPostcode} onChange={v => set('addl_zipPostcode', v)} />
          </Card>
          <Card title="Additional Insured — Boating Experience">
            <Field label="Years of Boating Experience" id="addl_experience" type="number" value={form.addl_experience} onChange={v => set('addl_experience', v)} />
  ...
  const step3 = (
    <div className="space-y-4">
      <Card title="Vessel Identity">
        <Grid2>
          <Field label="Vessel Name" id="vesselName" value={form.vesselName} onChange={v => set('vesselName', v)} required />
          <Field label="Hull / Serial Number" id="hullNumber" value={form.hullNumber} onChange={v => set('hullNumber', v)} />
        </Grid2>
        <Grid2>
          <Field label="Country of Flag" id="countryFlag" value={form.countryFlag} onChange={v => set('countryFlag', v)} />
          <Field label="State Flag (USA only)" id="stateFlagUSA" value={form.stateFlagUSA} onChange={v => set('stateFlagUSA', v)} />
        </Grid2>
      ...
      <Card title="Vessel Specifications">
        <Grid2>
          ...
          <Field label="Overall Length (metres)" id="length" value={form.length} onChange={v => set('length', v)} required placeholder="e.g. 12.5" />
        </Grid2>
        <Grid2>
  ...
  const step4 = (
    <div className="space-y-4">
      <Card title="Main Engine(s)">
        <Grid2>
          <Field label="Engine Manufacturer" id="engine_manufacturer" value={form.engine_manufacturer} onChange={v => set('engine_manufacturer', v)} />
          <Field label="Engine Model" id="engine_model" value={form.engine_model} onChange={v => set('engine_model', v)} />
        </Grid2>
        <Grid2>
          <Field label="Year Built" id="engine_yearBuilt" type="number" value={form.engine_yearBuilt} onChange={v => set('engine_yearBuilt', v)} />
          <Field label="Horsepower (HP)" id="engine_hp" value={form.engine_hp} onChange={v => set('engine_hp', v)} />
        </Grid2>
        <Grid2>
          <Select label="Fuel Type" id="engine_fuel" value={form.engine_fuel} onChange={v => set('engine_fuel', v)} options={['Diesel', 'Petrol', 'Electric', 'Hybrid', 'LPG']} />
          <Field label="Serial Number" id="engine_serial" value={form.engine_serial} onChange={v => set('engine_serial', v)} />
        </Grid2>
        <Grid2>
          <Field label="Number of Engines" id="engine_count" type="number" value={form.engine_count} onChange={v => set('engine_count', v)} />
          <Field label="Maximum Speed (knots)" id="engine_maxSpeed" value={form.engine_maxSpeed} onChange={v => set('engine_maxSpeed', v)} />
        </Grid2>
      </Card>
      ...
      {form.hasTender && (
        <>
          <Card title="Tender / Dinghy">
            <Grid2>
              <Field label="Hull / Serial Number" id="tender_serial" value={form.tender_serial} onChange={v => set('tender_serial', v)} />
              <YesNo label="Displays parent vessel name on hull?" id="tender_displaysName" value={form.tender_displaysName} onChange={v => set('tender_displaysName', v)} />
            </Grid2>
            <Grid2>
              <Field label="Manufacturer" id="tender_manufacturer" value={form.tender_manufacturer} onChange={v => set('tender_manufacturer', v)} />
              <Field label="Model" id="tender_model" value={form.tender_model} onChange={v => set('tender_model', v)} />
            </Grid2>
            <Grid2>
              <Field label="Length (metres)" id="tender_length" value={form.tender_length} onChange={v => set('tender_length', v)} />
          ...
          <Card title="Tender Outboard Motor">
            <Grid2>
              <Field label="Manufacturer" id="tenderMotor_manufacturer" value={form.tenderMotor_manufacturer} onChange={v => set('tenderMotor_manufacturer', v)} />
              <Field label="Model" id="tenderMotor_model" value={form.tenderMotor_model} onChange={v => set('tenderMotor_model', v)} />
            </Grid2>
            <Grid2>
              <Field label="Year Built" id="tenderMotor_yearBuilt" type="number" value={form.tenderMotor_yearBuilt} onChange={v => set('tenderMotor_yearBuilt', v)} />
              <Field label="Horsepower (HP)" id="tenderMotor_hp" value={form.tenderMotor_hp} onChange={v => set('tenderMotor_hp', v)} />
            </Grid2>
            <Grid2>
              <Select label="Fuel Type" id="tenderMotor_fuel" value={form.tenderMotor_fuel} onChange={v => set('tenderMotor_fuel', v)} options={['Petrol', 'Electric', 'Diesel']} />
              <Field label="Serial Number" id="tenderMotor_serial" value={form.tenderMotor_serial} onChange={v => set('tenderMotor_serial', v)} />
            </Grid2>
            <YesNo label="Locked when not in use?" id="tenderMotor_locked" value={form.tenderMotor_locked} onChange={v => set('tenderMotor_locked', v)} />
          </Card>
        </>
      )}
      ...
      {form.hasTrailer && (
        <Card title="Trailer">
          <Grid2>
            <Field label="Make / Model" id="trailer_makeModel" value={form.trailer_makeModel} onChange={v => set('trailer_makeModel', v)} />
            <Field label="Age (Years)" id="trailer_age" type="number" value={form.trailer_age} onChange={v => set('trailer_age', v)} />
          </Grid2>
          <Grid2>
            <Field label="Value" id="trailer_value" value={form.trailer_value} onChange={v => set('trailer_value', v)} placeholder="e.g. NZD 8,000" />
            <Field label="Serial Number" id="trailer_serial" value={form.trailer_serial} onChange={v => set('trailer_serial', v)} />
          </Grid2>
          <Grid2>
            <YesNo label="Wheel clamp fitted?" id="trailer_wheelClamp" value={form.trailer_wheelClamp} onChange={v => set('trailer_wheelClamp', v)} />
            <YesNo label="Locked in building when not in use?" id="trailer_lockedBuilding" value={form.trailer_lockedBuilding} onChange={v => set('trailer_lockedBuilding', v)} />
          </Grid2>
        </Card>
      )}
    </div>
  ...
  const safetyItems = [
    'Fire Extinguisher(s)', 'Smoke Detector(s)', 'Radar', 'GPS / Chart Plotter',
    'Depth Finder / Sounder', 'Auto Pilot', 'Engine Alarm System', 'VHF Radio',
    'Theft / Intruder Alarm', 'Tracking Device (GPS)', 'Surveillance System',
    'Locked / Fenced Enclosure', 'Secured Building / Shed',
  ...
  const step5 = (
    <div className="space-y-4">
      <Card title="Safety & Security Equipment On Board">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
          {safetyItems.map(item => (
            <CheckBox key={item} label={item} checked={form.safetyEquipment.includes(item)} onChange={() => toggleArr('safetyEquipment', item)} />
          ))}
        </div>
        <Field label="Other Safety Equipment (describe)" id="safetyOther" value={form.safetyOther} onChange={v => set('safetyOther', v)} />
        <Grid2>
          <Field label="Number of Handheld Fire Extinguishers" id="handheldExtCount" type="number" value={form.handheldExtCount} onChange={v => set('handheldExtCount', v)} />
          <div>
            <p className={labelCls}>Automatic Extinguisher Locations</p>
            <div className="space-y-2">
              {['Engine Room', 'Tank Space', 'Galley', 'Other'].map(loc => (
                <CheckBox key={loc} label={loc} checked={form.autoExtLocations.includes(loc)} onChange={() => toggleArr('autoExtLocations', loc)} />
              ))}
            </div>
          </div>
        </Grid2>
      ...
      <Card title="Gas System">
        ...
        {form.lpgUsed === 'Yes' && (
          <Grid2>
            <YesNo label="Gas cylinder stored in cockpit locker?" id="lpg_cockpitLocker" value={form.lpg_cockpitLocker} onChange={v => set('lpg_cockpitLocker', v)} />
            <YesNo label="Approved flexible delivery tubing installed?" id="lpg_approvedTubing" value={form.lpg_approvedTubing} onChange={v => set('lpg_approvedTubing', v)} />
          </Grid2>
        )}
      </Card>
      ...
      <Card title="Mooring & Storage">
        ...
        <Grid2>
          <Field label="Laid Up From (month)" id="laidUpFrom" value={form.laidUpFrom} onChange={v => set('laidUpFrom', v)} placeholder="e.g. June" />
          <Field label="Laid Up To (month)" id="laidUpTo" value={form.laidUpTo} onChange={v => set('laidUpTo', v)} placeholder="e.g. September" />
        </Grid2>
        <YesNo label="Is the vessel stored ashore on a daily basis?" id="storedAshoreDaily" value={form.storedAshoreDaily} onChange={v => set('storedAshoreDaily', v)} />
        {form.storedAshoreDaily === 'Yes' && (
          <Field label="How is it stored / secured ashore?" id="storedAshoreHow" value={form.storedAshoreHow} onChange={v => set('storedAshoreHow', v)} />
        )}
      </Card>
    </div>
  );
  ...
  const step6 = (
    <div className="space-y-4">
      ...
      <Card title="Coverage Required">
        <Grid2>
          <YesNo label="Fully Comprehensive" id="cover_fullyComp" value={form.cover_fullyComp} onChange={v => set('cover_fullyComp', v)} />
          <YesNo label="Third Party Liability Only" id="cover_tplOnly" value={form.cover_tplOnly} onChange={v => set('cover_tplOnly', v)} />
        </Grid2>
        <Grid2>
          <YesNo label="Private Pleasure Use Only" id="cover_privatePleasure" value={form.cover_privatePleasure} onChange={v => set('cover_privatePleasure', v)} />
          <YesNo label="Commercial Use" id="cover_commercial" value={form.cover_commercial} onChange={v => set('cover_commercial', v)} />
        </Grid2>
        {form.cover_commercial === 'Yes' && (
          <div className="pl-4 border-l-2 border-teal-700 space-y-3">
            <Grid2>
              <YesNo label="Bareboat Charter" id="cover_barebat" value={form.cover_barebat} onChange={v => set('cover_barebat', v)} />
              <YesNo label="Crewed Charter" id="cover_captainCharter" value={form.cover_captainCharter} onChange={v => set('cover_captainCharter', v)} />
            </Grid2>
            <Grid2>
              <YesNo label="Fishing Charter" id="cover_fishingCharter" value={form.cover_fishingCharter} onChange={v => set('cover_fishingCharter', v)} />
              <YesNo label="Other Commercial" id="cover_commercialOther" value={form.cover_commercialOther} onChange={v => set('cover_commercialOther', v)} />
            </Grid2>
            {form.cover_commercialOther === 'Yes' && (
              <Field label="Describe Other Commercial Use" id="cover_commercialOtherDetails" value={form.cover_commercialOtherDetails} onChange={v => set('cover_commercialOtherDetails', v)} />
            )}
            <YesNo label="Commercial Passenger Liability Required" id="cover_commercialPassenger" value={form.cover_commercialPassenger} onChange={v => set('cover_commercialPassenger', v)} />
            {form.cover_commercialPassenger === 'Yes' && (
              <Field label="Maximum Number of Passengers" id="cover_maxPassengers" type="number" value={form.cover_maxPassengers} onChange={v => set('cover_maxPassengers', v)} />
            )}
          </div>
        )}
        <Grid2>
          <YesNo label="In-Water Activities (snorkelling, diving)" id="cover_inWater" value={form.cover_inWater} onChange={v => set('cover_inWater', v)} />
          <YesNo label="Employed Captain / Crew" id="cover_employedCrew" value={form.cover_employedCrew} onChange={v => set('cover_employedCrew', v)} />
        </Grid2>
        {form.cover_employedCrew === 'Yes' && (
          <Field label="Maximum Number of Employed Crew" id="cover_maxCrew" type="number" value={form.cover_maxCrew} onChange={v => set('cover_maxCrew', v)} />
        )}
        <Grid2>
          <YesNo label="Water Skiing / Towing Activities" id="cover_waterSkiing" value={form.cover_waterSkiing} onChange={v => set('cover_waterSkiing', v)} />
          <YesNo label="Medical Expenses Cover" id="cover_medicalExp" value={form.cover_medicalExp} onChange={v => set('cover_medicalExp', v)} />
        </Grid2>
        <YesNo label="Salvage & Wreck Removal Cover" id="cover_salvage" value={form.cover_salvage} onChange={v => set('cover_salvage', v)} />
      </Card>

      <Card title="Special Endorsements">
        <p className="text-slate-400 text-sm mb-3">Select any additional endorsements you wish to request (subject to underwriter approval).</p>
        ...
        {form.end_racing === 'Yes' && (
          <Grid2>
            <TextArea label="Races / Events to be Covered" id="end_racingNames" value={form.end_racingNames} onChange={v => set('end_racingNames', v)} rows={2} placeholder="List race names and dates..." />
            <Field label="Mast & Boom Replacement Value" id="end_mastValue" value={form.end_mastValue} onChange={v => set('end_mastValue', v)} />
          </Grid2>
        )}
        {form.end_storm12plus === 'Yes' && (
          <YesNo label="Storm preparation plan in place?" id="end_stormPrep" value={form.end_stormPrep} onChange={v => set('end_stormPrep', v)} />
        )}
      </Card>
    </div>
  );
    ...
    <div className="space-y-4">
      ...
      <Card>
        <div className="space-y-5">
          <div className="space-y-2">
            <YesNo label="1. Has any insurer ever declined, cancelled or imposed special terms on any insurance for you or this vessel?" id="decl_declined" value={form.decl_declined} onChange={v => set('decl_declined', v)} />
            {form.decl_declined === 'Yes' && <TextArea label="Please provide details" id="decl_declinedDetails" value={form.decl_declinedDetails} onChange={v => set('decl_declinedDetails', v)} rows={3} required />}
          </div>
          <div className="space-y-2">
            <YesNo label="2. Have you or any operator of this vessel had any accidents, losses or claims in the last 5 years?" id="decl_accidents" value={form.decl_accidents} onChange={v => set('decl_accidents', v)} />
            {form.decl_accidents === 'Yes' && <TextArea label="Please provide full details (date, description, amount claimed)" id="decl_accidentDetails" value={form.decl_accidentDetails} onChange={v => set('decl_accidentDetails', v)} rows={3} required />}
          </div>
          <div className="space-y-2">
            <YesNo label="3. Have you or any operator been convicted of any offence involving dishonesty or fraud?" id="decl_dishonesty" value={form.decl_dishonesty} onChange={v => set('decl_dishonesty', v)} />
            {form.decl_dishonesty === 'Yes' && <TextArea label="Please provide details" id="decl_dishonestyDetails" value={form.decl_dishonestyDetails} onChange={v => set('decl_dishonestyDetails', v)} rows={3} required />}
          </div>
          <div className="space-y-2">
            <YesNo label="4. Is the vessel subject to any mortgage, finance agreement, or loan?" id="decl_mortgage" value={form.decl_mortgage} onChange={v => set('decl_mortgage', v)} />
            {form.decl_mortgage === 'Yes' && <TextArea label="Please provide lender name, loan amount and reference" id="decl_mortgageDetails" value={form.decl_mortgageDetails} onChange={v => set('decl_mortgageDetails', v)} rows={2} required />}
          </div>
          <div className="space-y-2">
            <YesNo label="5. Are you the sole owner of the vessel?" id="decl_soleOwner" value={form.decl_soleOwner} onChange={v => set('decl_soleOwner', v)} />
            {form.decl_soleOwner === 'No' && <TextArea label="Please provide details of other owners" id="decl_otherOwners" value={form.decl_otherOwners} onChange={v => set('decl_otherOwners', v)} rows={2} required />}
          </div>
          <div className="space-y-2">
            <YesNo label="6. Do you have any physical disability or medical condition that may affect your ability to operate this vessel?" id="decl_disabilities" value={form.decl_disabilities} onChange={v => set('decl_disabilities', v)} />
            {form.decl_disabilities === 'Yes' && <TextArea label="Please provide details" id="decl_disabilityDetails" value={form.decl_disabilityDetails} onChange={v => set('decl_disabilityDetails', v)} rows={2} required />}
          </div>
        </div>
      </Card>
    ...
    <div className="min-h-screen bg-slate-950 pb-16">
      ...
      <div className="bg-slate-900 border-b border-slate-800 py-6 px-4">
        <div className="max-w-2xl mx-auto">
          ...
          <p className="text-slate-400 text-sm mt-1">Complete all sections. Your progress is saved automatically.</p>
        </div>
      </div>
      ...
      <div className="bg-slate-900 sticky top-0 z-10 border-b border-slate-800 px-4 py-4">
        <div className="max-w-2xl mx-auto">
          <div className="flex justify-between text-xs text-slate-500 mb-2">
            <span>Step {step} of 7 — <span className="text-teal-400 font-medium">{STEPS[step - 1]}</span></span>
