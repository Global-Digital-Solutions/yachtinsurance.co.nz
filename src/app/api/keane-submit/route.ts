...
export async function POST(req: NextRequest) {
  try {
    ...
    // Build Keane payload — ONLY fields confirmed valid by Keane API (422 response strips unknowns).
    // Any field name not matching Keane's schema causes the ENTIRE payload to be rejected.
    // All form data is preserved in Supabase and the email backup regardless.
    // TODO: obtain Keane API docs to add remaining fields (engine, tender, trailer, SI detail, endorsements, declaration).
    const keanePayload: Record<string, unknown> = {
      ...
      vessel_is_conversion_details: (formData as Record<string, unknown>).isConversionDetails as string || '',
      ...
      mooring_location_postcode: (formData as Record<string, unknown>).mooringPostcode as string || '',
      mooring_vessel_ashore: yn(formData.storedAshoreDaily),

      // ── Sums insured (only TPL confirmed valid) ──────────────
